...
class Basic_AI():
    """Funktsioon keskmise tugevusega vastase mängimiseks"""
    def __init__(self, side):
        self.side = side # Mis poolel arvuti mängib

        #Defineerime lambdafunktsioonid sirgete ja diagonaalsete järjestuste testimiseks
        self.straight = lambda x1, x2: x1 == x2 
        self.diagonal = lambda pos1, pos2: abs(pos1[0]-pos2[0]) == abs(pos1[1]-pos2[1]) 
    def move(self, grid):
        #Põhiline käikude tegemise loogika
        myTokens = [] #Käidud oma nuppe
        enemyTokens = [] # käidud vastase nupud
        free = [] # vabad ruudud mänguväljal

        #Leiame nuppude arvu ja positsiooni mänguväljal
        for x in range(3):
            for y in range(3):
                member = grid[x][y]
        ...
        #Kas me saame sellel käigul võita
        solution = self.winning_move(grid, myTokens, free)
        if solution[0] == True:
            return solution[1]
        #Kas vastane saab järgmise käiguga võita
        solution = self.winning_move(grid, enemyTokens, free)
        if solution[0] == True:
            return solution[1]
        #Kas saab vastase lõksu panna
        ...
        if solution[0] == True:
            return solution[1]
        #Mõistlikud alustus positsioonid
        solution = self.defaults(grid)
        if solution[0] == True:
            return solution[1]

        #Kui pole ühtefi eelnevatest valime ühe vaba ruudu
        cell = random.choice(free) 
        return cell[2]
    def winning_move(self, grid, tokens, free):
        #Kontrollime kas käiguga saab võita
        for i in range(len(tokens)):
            token = tokens.pop()
            for other in tokens:
                if self.straight(token[0], other[0]): #Kas võita saab reaga
                    for i in [0,1,2]:
                ...
                if self.straight(token[1], other[1]): #Kas võita saab veeruga
                    for i in [0,1,2]: 
                ...
                if self.diagonal((token[0], token[1]), (other[0], other[1])): #kas võita saab diagonaaliga
                    for i in [0,1,2]:
                        if i not in (other[0], token[0]):
                            x = i
                        if i not in (other[1], token[1]):
    ...
    def traping_move(self, grid, tokens, free):
        for place in free:
            suitables = [] #Kas on sobilikke ruute vaenlase lõksu püüdmiseks vabade ruutude hulgas
            for token in tokens:
